...
* 操作数设置：在上方左侧下拉框选择字节数，文本框可以输入十六进制数字，右侧为字节序开关，开关打开可以将十六进制字节倒序，最右侧可以设置字节排序方式，可以选择按字节、字或双字交换位置
  > 输入框的数字默认为4位，转为二进制即16位，通过改变字节数可以改变二进制区域的位数
  >
  > 设置字节序仅影响下方二进制区排列方式，不会影响原始的十六进制数值
  >
